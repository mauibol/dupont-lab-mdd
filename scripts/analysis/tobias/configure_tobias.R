...
# Same thing but on neurogenic clusters for comparison with each other
cellTypes <- c('NSC','INP','NB','ImGC.2','ImGC.1','mGC')
grs <- CallPeaks(atac_merge, group.by = 'neuro_annotations', idents=cellTypes, macs2.path = MACS_2_PATH, combine.peaks=TRUE)

# Save bed files
setwd('/groups/mb928_gp/mp4486/tobias')
...
for (i in 1:length(grs)) {
  ...
  path = paste0("neurogenesis/peaks/trajectory_ALL.bed")
  write.table(gr_df, file = path, sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
}

for (c in cellTypes) {
  for (s in split) {
    cells <- s$annotations_broad[s$annotations_broad == c]
    if (length(cells) == 0) {
      print(paste('There are 0 cells in', c, "cluster of sample", s$sample[1], "skipping this sample..."))
      next
    }
    print(paste('There are', length(cells), 'cells in', c, "cluster of sample", s$sample[1]))
    print('Reading barcodes')
    srat <- subset(s, subset = annotations_broad == c)
    print(dim(srat))
    barcodes <- as.list(srat$gex_barcode)
    print(length(barcodes))
    barcodes <- lapply(barcodes, function(x) paste0("CB:Z:", x))
