...
```{r}
wilcox.test(
  snMultiome_Median_RNA_Count ~ Group,
  data = boxplot_data,
  exact = FALSE
)
```
...
```{r}
p_box_median_rna = ggplot(boxplot_data, aes(x = Group, y = snMultiome_Median_RNA_Count, fill = Group, color = Group)) +
  ...
  scale_y_continuous(
   ...
   annotate("text", x = 2, y = 16500, label= "atop(bolditalic('Wilcoxon p'==0.70), bolditalic('n'==30))", parse = TRUE) +
  ...
  labs(y = "snMultiome Median RNA Counts",
       x = NULL) +
  guides(
  fill = guide_legend(
    order = 1,
...
```{r}
ggsave("final_graph/boxplot_snMultiome_Median_RNA_Count MDD vs CTRL.pdf",p_box_median_rna, dpi = 72, width = 4, height = 4)
```
